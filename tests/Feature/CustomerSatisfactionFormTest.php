...
class CustomerSatisfactionFormTest extends TestCase
{
    ...
    public function test_jalali_dates_are_normalized_and_stored(): void { $this->post($this->user('customer_review'),$this->buyer(['submitted_at_fa'=>' ۱۴۰۳-۰۲-۰۵ ','shipment_sent_at_fa'=>'١٤٠٣/٠٢/٠٦']))->assertSessionHasNoErrors(); $this->assertDatabaseHas('customer_satisfaction_forms',['submitted_at'=>'2024-04-24','shipment_sent_at'=>'2024-04-25']); }
    public function test_invalid_jalali_date_is_rejected_on_its_field(): void { $this->post($this->user('customer_review'),$this->buyer(['submitted_at_fa'=>'۱۴۰۳/۱۳/۰۱']))->assertSessionHasErrors('customers.0.submitted_at_fa'); }
    public function test_empty_optional_shipment_date_is_accepted(): void { $this->post($this->user('customer_review'),$this->buyer(['shipment_sent_at_fa'=>'']))->assertSessionHasNoErrors(); }
}
