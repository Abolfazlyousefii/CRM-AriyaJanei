...
class CustomerSatisfactionFormTest extends TestCase
{
    ...
    private function submitForm(User $u,array $customer) { return $this->actingAs($u)->post(route('customer-satisfaction-forms.store'),['customers'=>[$customer]]); }

    public function test_customer_review_can_store_purchased_form(): void { $this->submitForm($this->user('customer_review'),$this->buyer())->assertSessionHasNoErrors(); $this->assertDatabaseHas('customer_satisfaction_forms',['purchase_status'=>'purchased']); }
    public function test_customer_review_can_store_not_purchased_form(): void { $this->submitForm($this->user('customer_review'),$this->nonBuyer())->assertSessionHasNoErrors(); $this->assertDatabaseHas('customer_satisfaction_forms',['no_purchase_reason'=>'price']); }
    public function test_user_without_role_cannot_store(): void { $this->submitForm(User::factory()->create(),$this->buyer())->assertForbidden(); }
    public function test_reason_is_required_for_not_purchased(): void { $this->submitForm($this->user('customer_review'),$this->nonBuyer(['no_purchase_reason'=>null]))->assertSessionHasErrors('customers.0.no_purchase_reason'); }
    public function test_reason_is_cleared_for_purchased(): void { $this->submitForm($this->user('customer_review'),$this->buyer(['no_purchase_reason'=>'price'])); $this->assertDatabaseHas('customer_satisfaction_forms',['purchase_status'=>'purchased','no_purchase_reason'=>null]); }
    public function test_buyer_questions_are_required(): void { $this->submitForm($this->user('customer_review'),['customer_full_name'=>'الف','purchase_status'=>'purchased'])->assertSessionHasErrors('customers.0.sales_response_score'); }
    public function test_score_below_one_is_rejected(): void { $this->submitForm($this->user('customer_review'),$this->buyer(['sales_response_score'=>0]))->assertSessionHasErrors('customers.0.sales_response_score'); }
    public function test_score_above_five_is_rejected(): void { $this->submitForm($this->user('customer_review'),$this->buyer(['shipping_time_score'=>6]))->assertSessionHasErrors('customers.0.shipping_time_score'); }
    public function test_invalid_yes_no_is_rejected(): void { $this->submitForm($this->user('customer_review'),$this->buyer(['would_recommend'=>'maybe']))->assertSessionHasErrors('customers.0.would_recommend'); }
    public function test_invalid_support_feature_is_rejected(): void { $this->submitForm($this->user('customer_review'),$this->buyer(['support_positive_features'=>['invalid']]))->assertSessionHasErrors('customers.0.support_positive_features.0'); }
    ...
    public function test_jalali_dates_are_normalized_and_stored(): void { $this->submitForm($this->user('customer_review'),$this->buyer(['submitted_at_fa'=>' ۱۴۰۳-۰۲-۰۵ ','shipment_sent_at_fa'=>'١٤٠٣/٠٢/٠٦']))->assertSessionHasNoErrors(); $this->assertDatabaseHas('customer_satisfaction_forms',['submitted_at'=>'2024-04-24','shipment_sent_at'=>'2024-04-25']); }
    public function test_invalid_jalali_date_is_rejected_on_its_field(): void { $this->submitForm($this->user('customer_review'),$this->buyer(['submitted_at_fa'=>'۱۴۰۳/۱۳/۰۱']))->assertSessionHasErrors('customers.0.submitted_at_fa'); }
    public function test_empty_optional_shipment_date_is_accepted(): void { $this->submitForm($this->user('customer_review'),$this->buyer(['shipment_sent_at_fa'=>'']))->assertSessionHasNoErrors(); }
}
