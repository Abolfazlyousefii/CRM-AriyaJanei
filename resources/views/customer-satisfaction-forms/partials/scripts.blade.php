...
document.querySelector('#add-customer-btn')?.addEventListener('click',()=>{const nextIndex=container.children.length;container.insertAdjacentHTML('beforeend',template.innerHTML.replaceAll('__INDEX__',nextIndex).replaceAll('__INDEX_DISPLAY__',nextIndex+1));renumber();window.jalaliDatepicker?.startWatch()});
