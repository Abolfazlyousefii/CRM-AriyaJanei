		...
		<div class="flex justify-between items-center mx-header__row">
			<div class="flex items-center gap-3">
				...
				@php
					...
					$token= app(\App\Services\TokenService::class)->hash($phone);
					$value = Arr::pull($token, 'string');
					Arr::set($token, 'phone', $value)
                @endphp
				<a href="{{ env('INVENTORY_URL') }}/auth/phone/verify?{{Arr::query($token)}}" target="_blank" class="btn btn-primary d-none d-sm-inline-flex align-items-center gap-2 mx-header__dashboard-btn {{ request()->routeIs('dashboard') ? 'active' : '' }}">
					<i class="bi bi-speedometer2"></i>
					<span>ورود به انبار داری</span>
				</a>
			...
			<div class="d-none d-sm-flex mx-header__actions">
				...
				@auth
					...
					<button type="button" class="btn  mx-icon-btn" onclick="window.history.back()" aria-label="بازگشت" title="بازگشت" style:
					"background-color:="" #0f172a"="" style="background-color: #3d3d3d !important;">
			...
			<div class="modal-body">
				...
				<div data-msg-list>
					@forelse(($headerMessages ?? collect()) as $messageItem)
						...
						<button type="button" data-msg-item data-chat-target="{{ $otherUser->id }}" data-user-name="{{ $otherUser->name }}">
					...
					@if(($headerMessageConversations ?? collect())->count() > 0)
						@foreach($headerMessageConversations as $threadUserId => $threadMessages)
							...
							@if($threadUser)
								...
								<div data-msg-thread="{{ $threadUserId }}" data-user-name="{{ $threadUser->name }}" data-user-avatar="{{ $threadAvatar }}" style="{{ $firstThreadUser && (int) $firstThreadUser->id === (int) $threadUserId ? '' : 'display:none;' }}">
									...
									<div data-msg-compose>
										<form action="{{ route('messages.reply', $threadUserId) }}" method="POST" data-msg-compose-form data-thread-user-id="{{ $threadUserId }}">
											@csrf
